  ...
  return (
    <main className="min-h-screen bg-[#1c1a17] text-[#f4ecd8] font-sans selection:bg-[#E0005C] selection:text-white relative overflow-x-hidden">
      ...
      <header className="fixed top-0 left-0 right-0 z-40 bg-[#1c1a17]/90 backdrop-blur-md border-b border-[#f7ac32]/20">
        <div className="flex items-center justify-between px-4 md:px-6 py-3 max-w-7xl mx-auto relative h-full">
          {/* LEFT: Menu / Close Button */}
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="z-50 mix-blend-difference text-[#f7ac32] hover:text-white transition-colors cursor-pointer w-10 h-10 flex items-center justify-start">
            {isMenuOpen ? <X className="w-8 h-8" /> : <Menu className="w-8 h-8" />}
          </button>

          {/* CENTER: Logo */}
          <div 
            className="text-xl md:text-2xl font-black uppercase tracking-[0.2em] text-[#f7ac32] absolute left-1/2 -translate-x-1/2 cursor-pointer hover:text-white transition-colors flex items-center gap-2 z-10"
            onClick={navToHome}
          >
          ...
          <div className="flex items-center gap-4 z-50">
            {/* Desktop Only Nav (Optional, user said 'like before' - checking previous snapshots) */}
            {!isMenuOpen && (
              <div className="hidden lg:flex items-center gap-6 mr-4 text-[10px] font-bold uppercase tracking-[0.2em] text-[#f4ecd8]/40">
            ...
            <button 
              ...
              className="px-4 py-2 font-black uppercase tracking-widest cursor-pointer transition-all text-xs bg-[#f7ac32] text-[#1c1a17] hover:bg-white hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(247,172,50,0.2)]"
            >
              Search
            </button>
          </div>
